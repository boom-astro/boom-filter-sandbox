/**
 * Spherical astronomy on the client: angular separations, and equatorial to
 * galactic coordinates.
 *
 * These mirror the server-side implementations in `src/utils` so a page can
 * annotate what it has already fetched without another round trip.
 */

const { PI } = Math;
const rad = PI / 180;

/** Angular separation between two equatorial positions, in `unit`. */
...
/**
 * Rotation from equatorial (ICRS) to galactic axes.
 *
 * The rows are the galactic x, y, and z axes expressed as unit vectors in
 * equatorial coordinates, so multiplying a position by this matrix re-expresses
 * it in the galactic frame.
 */
const RGE = [
...
/** Convert equatorial ra/dec to galactic longitude and latitude, in degrees. */
